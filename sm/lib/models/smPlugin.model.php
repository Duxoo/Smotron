<?php
class smPluginModel extends smSortableModel
{
    const TYPE_SHIPPING = 'shipping';
    const TYPE_PAYMENT = 'payment';
    protected $table = 'sm_plugin';
    ...
    public function listPlugins($type, $options = array())
    {
        ...
        if (!empty($options[$complementary])) {
            $non_available = smHelper::getDisabledMethods($type, $options[$complementary]);
        }
    ...
    public function deleteByField($field, $value = null)
    {
        ...
        if ($ids) {
            if ($res = parent::deleteByField($this->id, $ids)) {
                $model = new smPluginSettingsModel();
