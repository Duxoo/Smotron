 ...
 */
class smReportsSalesAction extends waViewAction
{
    private $max_n_graphs = 20;

    public function execute()
    {
        smReportsSalesAction::jsRedirectIfDisabled();

        $metrics_model = new smMetricsModel();

        $def_cur = 'RUB';

        $sales_model = new smMetricsSalesModel();
        ...
        $graph_data = self::getGraphData($sales_model->getPeriodByDate($start_date, $end_date, $model_options + $default_options));

        $table_data = $sales_model->getPeriod( $start_date, $end_date,$total_rows);
    ...
    public static function getGraphData($sales_by_day)
    {
        ...
        foreach ($sales_by_day as &$d) {
            ...
            $graph_row['loss'] = ifset($d['ref'], 0); // profit can be negative; it renders as a red loss below zero
    ...
    {
        return wa('sm')->getConfig()->getOption('reports_simple');
    }
