(function($) {
    ...
    $.smotron = {
        ...
        init: function(options) {
            ...
            if (options.menu_floating) {
                ...
                $('body').on('click', 'a.js-print', function() {
                    return $.smotron.helper.print(this);
                });
            }
        },
        ...
        getMethod: function(args, scope, name) {
            ...
            if (args.length) {
                ...
                while (chunk = args.shift()) {
                    ...
                    $.smotron.trace('$.getMethod try', [name, callable, args]);
                    if (callable === true) {
                        method.name = name;
        ...
        updateAppCounter: function(count) {
            count = parseInt(count, 10) || '';
            var counter = $('#wa-app-post').find('.indicator');
            if (!counter.length) {
                $('#wa-app-post').find('a').append('<span class="indicator" style="display:none;"></span>');
                counter = $('#wa-app-post').find('.indicator');
            }
            counter.text(count);
            if (count) {
                counter.show();
            ...
            var handler = function() {
                ...
                timer_id = setTimeout(function() {
                    ...
                    if (val.length > threshold && field.is('input')) {
                        var textarea = $.smotron.input2textarea(field);
                        textarea.css('height', height);
                        field.replaceWith(textarea);
                        field = textarea;
                        field.focus();
                    } else if (val.length <= threshold && field.is('textarea')) {
                        var input = $.smotron.textarea2input(field);
                        input.css('height', '');
        ...
        helper: {
            ...
            print: function(el) {
                ...
                if ($body.find('.map[id^="yandex-map"]').length) {
                    ...
                    $head.append('<script src="'+$.smotron.options.jquery_url+'"></script>');
                    wnd.ymaps = undefined;
                }
